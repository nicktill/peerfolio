...
export const getAccountTypeIcon = (type: string, subtype?: string) => {
  ...
  if (accountType.includes("saving"))
    return { icon: "💰", color: "from-green-500 to-green-600", bgColor: "bg-green-50" }
  if (accountType.includes("checking")) return { icon: "🏦", color: "from-blue-500 to-blue-600", bgColor: "bg-blue-50" }
  if (accountType.includes("investment") || accountType.includes("brokerage"))
    return { icon: "📈", color: "from-purple-500 to-purple-600", bgColor: "bg-purple-50" }
  if (accountType.includes("401k") || accountType.includes("retirement"))
    return { icon: "🏛️", color: "from-orange-500 to-orange-600", bgColor: "bg-orange-50" }
  if (accountType.includes("hsa")) return { icon: "🏥", color: "from-teal-500 to-teal-600", bgColor: "bg-teal-50" }
  if (accountType.includes("cash management") || accountType.includes("money market"))
    return { icon: "💵", color: "from-emerald-500 to-emerald-600", bgColor: "bg-emerald-50" }
  if (accountType.includes("cd") || accountType.includes("certificate"))
    return { icon: "🏆", color: "from-yellow-500 to-yellow-600", bgColor: "bg-yellow-50" }
  if (accountType.includes("loan") || accountType.includes("mortgage"))
    return { icon: "🏠", color: "from-indigo-500 to-indigo-600", bgColor: "bg-indigo-50" }
...
// Function to get portfolio vs total balances with proper liability handling
export const getBalanceSummary = (accounts: any[]) => {
  ...
  accounts.forEach((account) => {
    ...
    const category = account.category || getAccountCategory(account.type)
  ...
  return {
    portfolioAssetsValue: totalAssets, // Assets only (for percentage calculations)
...
const getAccountCategory = (accountType: string) => {
  ...
  if (type.includes("saving") || type.includes("checking") || type.includes("cash") || type.includes("money market")) {
    return "assets"
  }
...
export const getTopHoldings = (
  ...
  if (!isDemoMode && (plaidData || connectedPlaidAccounts.length > 0)) {
    ...
    const validHoldings = plaidData?.holdings?.filter((holding: any) => (holding.market_value || 0) > 0) || []

    if (validHoldings.length > 0) {
      // Use real stock holdings with actual values (only if we have real holdings)
      return validHoldings.slice(0, 5).map((holding: any) => {
